...
// =================================================================================
// [配置] 部署版本控制
// 修改此版本号(或时间戳)并重新部署，会触发系统维护任务(DB迁移 & WebDAV推送)
// =================================================================================
const DEPLOY_VERSION = 'v1.0.2_20241210_fix_db'; 
...
// =================================================================================
// [重构] 系统自动维护任务 (数据库迁移 & WebDAV 推送)
// =================================================================================

async function executeSystemTasks(c, force = false) {
    try {
        const kv = c.env.CONFIG_KV;
        const kvKey = 'system_deployed_version';
        
        // 检查版本号，决定是否执行维护任务
        if (!force) {
            const lastVersion = await kv.get(kvKey);
            if (lastVersion === DEPLOY_VERSION) {
                return; // 版本未变，跳过
            }
        }

        console.log(`[System] 检测到新版本/强制执行 (${DEPLOY_VERSION})，开始维护任务...`);

        // 1. [核心修复] 自动执行数据库迁移 (InitDB)
        // 这确保了新代码部署后，数据库结构会自动更新，无需手动访问 /setup
        try {
            const db = c.get('db');
            if (db) {
                await db.initDB();
                console.log('[System] 数据库自动迁移完成');
            }
        } catch (dbErr) {
            console.error('[System] 数据库迁移失败:', dbErr);
            // 数据库迁移失败属于严重错误，但不应阻断 WebDAV 推送的尝试
        }
        ...
        if (WEBDAV_CONFIG.URL && WEBDAV_CONFIG.USER && WEBDAV_CONFIG.PASS) {
            ...
            if (res.ok) {
                console.log(`[System] WebDAV 推送成功: ${fileName}`);
            } else {
                console.error(`[System] WebDAV 推送失败: ${res.status}`);
            }
        }

        // 3. 更新 KV 中的版本号，标记任务完成
        await kv.put(kvKey, DEPLOY_VERSION);
        return { success: true };

    } catch (e) {
        console.error('[System] 维护任务异常:', e);
...
app.onError((err, c) => {
    ...
    // [优化] 针对 "no such column" 错误提供友好的提示
    if (err.message && err.message.includes('no such column')) {
        const msg = `数据库结构不匹配 (缺少列)。\n请尝试访问 ${new URL(c.req.url).origin}/setup 以手动修复数据库。`;
        if (c.req.path.startsWith('/api') || c.req.header('accept')?.includes('json')) {
            return c.json({ success: false, message: msg, error: err.message }, 500);
        }
        return c.text(`❌ 系统错误:\n${msg}\n\n详细错误: ${err.message}`, 500);
    }
...
app.use('*', async (c, next) => {
    ...
    try {
        ...
        if (!path.startsWith('/setup')) {
            // [修改] 调用新的系统维护函数
            c.executionCtx.waitUntil(executeSystemTasks(c));
        } 
...
app.get('/api/trigger-push', async (c) => {
    ...
    const result = await executeSystemTasks(c, force);
...
export default {
    ...
    async scheduled(event, env, ctx) {
        ...
        const configManager = new ConfigManager(env.CONFIG_KV);
        const config = await configManager.load();
        
        ctx.waitUntil((async () => {
            try {
                await data.cleanupExpiredTokens(db);
                console.log('[Cron] Token 清理完成');
            } catch (e) {
                console.error('[Cron] Token 清理失败:', e);
            }
        })());
        ...
        ctx.waitUntil((async () => {
            try {
                let storage = null;
                if (config.storageMode) {
                     storage = initStorage(config, env);
                }
                
                if (storage) {
                    await data.cleanupTrash(db, storage); 
                    console.log('[Cron] 回收站清理完成');
                }
            } catch (e) {
                console.error('[Cron] 回收站清理/存储初始化失败:', e);
            }
        })());
    }
};
